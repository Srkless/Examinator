...
/**
 * REST controller for managing subjects and their related operations. Handles CRUD operations,
 * user-subject associations, and subject retrieval.
 */
@RestController
...
public class SubjectController {
    ...
    /**
     * Retrieves a list of all subjects associated with the currently authenticated user.
     *
     ...
     * @return 200 on success; 404 if user or subject not found; 500 on unexpected errors
     */
    ...
    public ResponseEntity<?> addProfessor(@RequestBody AddProfessorRequest request) {
        try {
            ...
            subjectService.addUserToSubject(username, subjectCode);
    ...
    /**
     * Remove a professor (user) from an existing subject.
     *
     * <p>Expects a JSON body:
     *
     * <pre>
     ...
     * </pre>
     *
     * @param request the request containing the username and subject code
     * @return 200 on success; 404 if user or subject not found; 500 on unexpected errors
     */
    @Operation(summary = "Remove a professor (user) from a subject")
    @ApiResponse(responseCode = "200", description = "User added to subject")
    @ApiResponse(responseCode = "404", description = "User or subject not found")
    @ApiResponse(responseCode = "500", description = "Internal server error")
    @PostMapping("/removeProfessor")
    public ResponseEntity<?> removeProfessor(@RequestBody AddProfessorRequest request) {
        try {
            String username = request.getUsername();
            Integer subjectCode = request.getSubjectCode();

            subjectService.removeUserFromSubject(username, subjectCode);
            return ResponseEntity.ok("User successfully removed from subject.");
        } catch (RuntimeException e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(e.getMessage());
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
    ...
    @Operation(summary = "Get professors by subject ID")
    @ApiResponse(
            responseCode = "200",
            description = "List of professors for the subject",
            content =
                    @Content(
                            array = @ArraySchema(schema = @Schema(implementation = UserDTO.class))))
    @ApiResponse(responseCode = "404", description = "Subject not found")
    @ApiResponse(responseCode = "500", description = "Internal server error")
    @GetMapping("/{subjectCode}/users")
    public ResponseEntity<?> getUsersBySubjectCode(@PathVariable Integer subjectCode) {
        try {
            List<UserDTO> users = subjectService.getProfessors(subjectCode);
     ...
     *
     * <p>Expects a JSON body with key-value pairs like:
     *
     * <pre>
     * {
     *   "name": "Mathematics",
     ...
     *
     * <p>Expects a full JSON representation of a {@link Subject} object. The object must contain a
     * valid ID to be updated.
     *
     * @param updated the subject with updated fields
     * @return 200 with updated subject; 400 on invalid input; 404 if subject not found; 500 on
     *     unexpected errors
     */
