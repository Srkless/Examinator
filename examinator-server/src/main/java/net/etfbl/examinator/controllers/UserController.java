...
public class UserController {
   ...
   * @return 200 OK if registration successful, 400 Bad Request with error message
   *         otherwise.
   */
  @Operation(summary = "Register a new user")
  @ApiResponses({
      @ApiResponse(responseCode = "200", description = "User registered successfully"),
      @ApiResponse(responseCode = "400", description = "Registration failed", content = @Content(schema = @Schema(implementation = String.class)))
  })
  @PostMapping("/register")
  public ResponseEntity<?> register(
      @Parameter(description = "User registration data", required = true) @RequestBody Map<String, String> body) {
  ...
  @ApiResponses({
      @ApiResponse(responseCode = "200", description = "Authentication successful", content = @Content(schema = @Schema(implementation = Map.class))),
  ...
  public ResponseEntity<?> login(
      @Parameter(description = "User login data", required = true) @RequestBody Map<String, String> body) {
