...
const ResultsForm = () => {
  ...
  const { subject, selectedSchoolYear, selectedActivity } = location.state || {};
  ...
  useEffect(() => {
    const fetchActivityData = async () => {
      try {
        ...
        const selected = data.activities.find(a => a.name === selectedActivity);
        ...
        setSaveStatus({ type: 'error', message: 'Greška pri učitavanju podataka' });
      }
    };
  ...
  const loadExistingResults = async (activityId, studentList) => {
    try {
      ...
      for (const student of studentList) {
        try {
          const response = await fetch(`/api/results/${student.id}/${activityId}`, {
            headers: {
          ...
          if (response.ok) {
            ...
            existingResults[student.index] = result.points.toString();
          }
        } catch (error) {
          // Result doesn't exist yet, which is fine
          console.log(`No existing result for student ${student.index}`);
        }
      }
    ...
    reader.onload = (event) => {
      ...
      lines.forEach(line => {
        ...
        if (index && score) {
          ...
          if (!isNaN(numScore)) {
            const clampedScore = Math.max(0, Math.min(numScore, maxPoints));
  ...
  const saveResults = async () => {
    if (!activityData?.id) {
      setSaveStatus({ type: 'error', message: 'Nedostaju podaci o aktivnosti' });
      return;
    }
    ...
    try {
      const batchData = Object.entries(results)
        .filter(([studentIndex, points]) => points !== '' && points !== undefined)
        .map(([studentIndex, points]) => ({
          studentIndex,
          points: parseFloat(points),
          subjectCode: subject?.match(/\((\d+)\)/)?.[1] ? parseInt(subject.match(/\((\d+)\)/)[1]) : null
        }));

      if (batchData.length === 0) {
        setSaveStatus({ type: 'warning', message: 'Nema rezultata za čuvanje' });
        setLoading(false);
        return;
      }
      ...
      const response = await fetch(`/api/results/batch/${activityData.id}`, {
        method: 'POST',
        headers: headers,
      ...
      if (!response.ok) {
        if (response.status === 401) {
          setSaveStatus({ type: 'error', message: 'Nemate dozvolu za ovu operaciju.' });
          return;
        }
        if (response.status === 404) {
          setSaveStatus({ type: 'error', message: 'Aktivnost nije pronađena.' });
          return;
        }
  ...
  const filteredStudents = students.filter(
    s => `${s.firstName} ${s.lastName} ${s.index}`.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <div className="results-form">
      ...
      <main className="main-content results-container">
        <div className="info-row">
          ...
          <div>
            <strong>Školska godina:</strong> <span>{selectedSchoolYear}</span>
          </div>
          <div>
            <strong>Aktivnost:</strong> <span>{selectedActivity}</span>
            {activityData && <span> (max {activityData.maxPoints} poena)</span>}
          </div>
          <div>
            <label htmlFor="upload-results" className="upload-btn">
        ...
        {saveStatus.message && (
          <div className={`status-message ${getStatusClassName(saveStatus.type)}`}>
            {saveStatus.message}
          </div>
        )}
        ...
        <table>
          ...
          <tbody>
            {filteredStudents.length > 0 ? (
              ...
              <tr>
                <td colSpan="5" className="no-data">Nema studenata</td>
              </tr>
            )}
          </tbody>
        </table>
